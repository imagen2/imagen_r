...
deriveSST<-function(df) {
	if(!checkEnvironment()) {stop("Can't load required libraries!")}
...
#Clearly there are a some people who are simply not responding for big chunks of this task - this gives them a good stop hit rate but very bad go_success rate - they will probably need to be excluded - really this should not be happening in a supervised admin situation?
